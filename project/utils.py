...
class DIV2K_train_set(Dataset):
	def __init__(self, data_dir, upscale_factor=4, crop_size = 96):
		...
		self.lr_transform = transforms.Compose([
			transforms.Resize(self.crop_size//upscale_factor, interpolation=InterpolationMode.BICUBIC),
			transforms.ToTensor()
			])
...
class DIV2K_valid_set(Dataset):
	...
	def __getitem__(self, index):
		...
		lr_transform = transforms.Compose([
			transforms.Resize(crop_size// self.upscale_factor, interpolation=InterpolationMode.BICUBIC),
			transforms.ToTensor(),
			])
...
class FeatureExtractor(nn.Module):
	def __init__(self):
		super().__init__()
		vggnet = vgg19(weights=VGG19_Weights.IMAGENET1K_V1)
